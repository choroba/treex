...
sub process_atree
{
    ...
    # Even for individual syntactic fixes it may be better that they loop over
    # the tree themselves before the next fix is applied (often the construction
    # will affect multiple nodes, not just the one we are looking at now).
    ...
    # Some depictives in accusative with the conjunction "jako" slipped through
...
sub relabel_depictive_objects
{
    ...
    foreach my $node (@nodes)
    {
        ...
        {
            my $jako = any {$_->lemma() eq 'jako'} ($node->children());
            if($jako)
            {
...
BEGIN
{
    ...
    (
        ...
        ['míň než',            'fixed',   'méně než',           'ADV SCONJ',           'Dg-------2A---1 J,-------------',                 'pos=adv|polarity=pos|degree=cmp|extpos=adv pos=conj|conjtype=sub', '0:advmod 1:fixed'],
        ['méně než',           'fixed',   'méně než',           'ADV SCONJ',           'Dg-------2A---- J,-------------',                 'pos=adv|polarity=pos|degree=cmp|extpos=adv pos=conj|conjtype=sub', '0:advmod 1:fixed'],
